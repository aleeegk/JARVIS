...
# Configurar sys.path con la raíz del proyecto para importar config
...
CONFIG_DIR      = BASE_DIR.parent.parent / "config" if (BASE_DIR.parent.parent / "config").exists() else BASE_DIR / "config"
...
def _pcm16_level(pcm_bytes: bytes) -> float:
    """Cheap RMS amplitude (0..1) of a 16-bit PCM chunk — used to drive the HUD's
    audio-reactive speaking pulse. No numpy dependency, subsampled for speed."""
    if not pcm_bytes or len(pcm_bytes) < 2:
        return 0.0
    try:
        samples = array.array("h", pcm_bytes[: len(pcm_bytes) - (len(pcm_bytes) % 2)])
    except Exception:
        return 0.0
    n = len(samples)
    if n == 0:
    ...
    if cnt == 0:
        return 0.0
    rms = (acc / cnt) ** 0.5
    return max(0.0, min(1.0, rms / 11000.0))
CHUNK_SIZE          = 1024
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "osint_lookup",
        "description": (
            "Searches a username across hundreds of social media and web platforms "
            "using Sherlock to find where that username has a public profile. "
            "Use only for legitimate purposes: checking your own accounts, username "
            "availability, or security research — never to locate or harass someone "
...
class JarvisLive:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        identity_ctx = (
            ...
            f"Your developer and creator is karakas. If anyone asks who made you, "
            f"who developed you, or who your creator is, answer: karakas.\n"
            f"{_addr}\n\n"
        )
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "open_app":
                ...
                r = await loop.run_in_executor(None, lambda: osint_lookup(parameters=args, response=None, player=self.ui))
                result = r or "OSINT lookup complete."
    ...
    async def _play_audio(self):
        ...
        try:
            while True:
                ...
                try:
                    self.ui.set_level(_pcm16_level(bytes(batch)))
                except Exception:
                    pass
